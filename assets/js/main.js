...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (mobileMenuToggle && navMenu) {
            ...
            const handleHover = (dropdown, isEnter) => {
                const viewportWidth = window.innerWidth;
                if (viewportWidth > 992) {
                    dropdown.classList.toggle('open', isEnter);
                }
            };

            navMenu.querySelectorAll('.dropdown').forEach(dropdown => {
                dropdown.addEventListener('mouseenter', () => handleHover(dropdown, true));
                dropdown.addEventListener('mouseleave', () => handleHover(dropdown, false));
            });
        }
